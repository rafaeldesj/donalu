...
export const DeliveryActive = () => {
  ...
  // Busca coordenadas do endereço da entrega ativa
  useEffect(() => {
    if (!activeOrder) {
      setDestCoords(null);
      return;
  ...
  const AvailableOrderMap = ({ orderId, address, clientCoords }: AvailableOrderMapProps) => {
    const mapContainerRef = useRef<HTMLDivElement>(null);
    const mapInstanceRef = useRef<L.Map | null>(null);
    ...
    useEffect(() => {
      ...
      // Adiciona evento de clique para abrir o Google Maps com rota
      map.off('click');
      map.on('click', () => {
        window.open(
